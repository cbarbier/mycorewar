...
/*   Updated: 2017/10/03 16:31:02 by fmaury           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_check_error(char *file, t_asm *sfile)
{
	...
	{
		ft_putstr("Bad file extension, should be a '.cor'");
		return (0);
	}
...
void		ft_com_name(t_asm *sfile)
{
	...
	tmp = ft_strdup(sfile->bin + 11 + PROG_NAME_LENGTH);
...
int			ft_reverse(char *file, t_asm *sfile)
{
	...
	if (!ft_check_error(file, sfile))
		return (0);
	if ((ret = read(sfile->fd, sfile->bin, CHAMP_MAX_SIZE + PROG_NAME_LENGTH + COMMENT_LENGTH + 1)) < 0)
	{
